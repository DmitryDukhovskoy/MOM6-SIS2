...
! Debug - check pointer:
  iiG = 230 ; jjG = 700
  kk0 = 1
  if (trim(rlxfld_name) == 'part_size') kk0=0
  do i=isd,ied ; do j=jsd,jed
    iiG = G%isd_global + (i-1)  ; jjG = G%jsd_global + (j-1)
    if (i.eq.CS%itest .and. j.eq.CS%jtest) then
      do k=kk0,CatIce
        write(mesg,'("set_up_isponge: ",A," k=",i2," ispCS =",f12.5," IST =",f12.5)') &
             trim(rlxfld_name), k, CS%var(CS%fldno)%p(i,j,k), f_ptr(i,j,k)
        write(*,'(A)') trim(mesg)
      enddo
    endif
  enddo ; enddo
...
! First get relax fields and interp. in time:
  ...
  ! Input hice is "ice volume" per m2 (following CICE), i.e. hice=voli=sum(hice(k)*cice(k))
  ! scale ice thickness m --> kg m-2 and unscale US%m_to_Z
  ! call distribute_ice2cats_simple(CS, IG, G)
...
subroutine distribute_ice2cats_simple(CS, IG, G, scaled, eps_err)
  type(isponge_CS),        pointer     :: CS       !< A pointer that is set to point to the ice sponge control
                                                   !! structure for this module
  type(ice_grid_type),     intent(in)  :: IG       !< The sea-ice specific grid type
  type(SIS_hor_grid_type), intent(in)  :: G        !< The horizontal grid type
  ...
  character(len=40)  :: mdl = "distribute_ice2cats_simple"  ! This module's name.
  character(len=256) :: mesg
  logical :: scaled_hice
  ...
  do col=1,CS%num_col
    ...
    hice_k = hice/cice   ! ice volume (m3/m2) --> ice thickness (m)  in cat=k
    icat0 = find_icat(hice_k, CatIce, hLim_vals)
! Debug:
    !if (i.eq.CS%itest .and. j.eq.CS%jtest) then
    !  write(mesg,'(A," test pnt: hice=",f7.3," cice=",f6.3," hice_k=",f7.3,&
    !              " assigned cat=",i2,"(",f5.2,"m)")') &
    !        trim(mdl), hice, cice, hice_k, icat0, hLim_vals(icat0)
    !  write(*,'(A)') trim(mesg) 
    !endif
    ...
    do m=1,CS%fldno ; do k=1,CatIce
      if (k.eq.icat0) then
        select case (trim(CS%var(m)%fld_name))
          case('mH_ice')
            CS%Ref_val(m)%p(col,k) = hice_k*CS%Ref_val(m)%scale  !< scaled
          case('part_size')
            CS%Ref_val(m)%p(col,k) = cice
        end select
! Debug:
        if (i==CS%itest .and. j==CS%jtest) then
          write(mesg,'(A,": ",A," scale=",f10.4," ref_value=",f10.4)') &
               trim(mdl), trim(CS%var(m)%fld_name), CS%Ref_val(m)%scale, CS%Ref_val(m)%p(col,icat0)
          write(*,'(A)') trim(mesg)
        endif
      else
  ...
  if (allocated(cice_cat)) deallocate(cice_cat)
  if (allocated(hice_cat)) deallocate(hice_cat)
  
end subroutine distribute_ice2cats_simple

!> Better than simple redistribution of 2D hice and iconc into ice thickness categories
!! place all ice into 1 category based on original ice thickness (hice)
!! Fill the lower cats with "some" ice to avoid 0s ice allowing better convergence
!! of CICE ITD iteration algorithm
subroutine distribute_ice2cats(CS, IG, G, scaled, eps_err)
  ...
  isd = G%isd ; ied = G%ied ; jsd = G%jsd ; jed = G%jed
  ! Global indx, start pnts. data domain:
...
! Debug
    if (i.eq.CS%itest .and. j.eq.CS%jtest) then
      ...
      write(*,'(A)') trim(mesg)
      !do k=1,CatIce
      !  write(mesg,'("icat=",i2," mH_ice=",f7.3," part_area=",f6.3)') k, hcat(k), ccat(k)
      !  write(*,'(A)') trim(mesg)
      !enddo
    endif
...
!
! Debug: print ice thkn by cats and total at 1 point
